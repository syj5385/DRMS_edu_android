...
public class Cont1_4_Activity extends AppCompatActivity {
    ...
    ////////////////////////controll button & touch image///////////////////////
        ImageView left_roll, right_roll;
        EditText left_speed, right_speed;
        TextView left_send, right_send;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cont1_4);
        ...
        left_roll = (ImageView)findViewById(R.id.left_roll);
        left_roll.setOnTouchListener(mTouchListener);

        right_roll = (ImageView)findViewById(R.id.right_roll);
        right_roll.setOnTouchListener(mTouchListener);

        left_speed = (EditText)findViewById(R.id.left_vel);

        right_speed = (EditText)findViewById(R.id.right_vel);
        ...
        left_send = (TextView) findViewById(R.id.left_send);
        left_send.setOnTouchListener(mTouchListener);

        right_send = (TextView)findViewById(R.id.right_send);
        right_send.setOnTouchListener(mTouchListener);
        ...
        public boolean onTouch(View v, MotionEvent event) {
            switch(v.getId()){
                ...
                case R.id.left_send :
                    if(bluetoothService_obj.getState() == BluetoothService.STATE_CONNECTED){
                        if(event.getAction() == MotionEvent.ACTION_DOWN){
                            String speed_left = left_speed.getText().toString();
                            sendMessage("e"+speed_left+"\n",MODE_REQUEST);
                            left_speed.setText("");
                        }
                    }
                    break;
                ...
                case R.id.right_send :
                    if(bluetoothService_obj.getState() == BluetoothService.STATE_CONNECTED){
                        if(event.getAction() == MotionEvent.ACTION_DOWN){
                            String speed_right = right_speed.getText().toString();
                            sendMessage("f"+speed_right+"\n",MODE_REQUEST);
                            right_speed.setText("");
                        }
                    }
                    break;
    ...
    public class FiledownThread extends Thread{
        ...
        public void run() {
            try {
                InputStream is = getResources().openRawResource(R.raw.r1_4);

                File file = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)+"/drs1_4_firmware.hex");
