...
//        this.inputstream = inputstream;
        ...
        DRMS drms = (DRMS)mActivity.getApplication();
        if(request == DRMS.LEV3_DRONE){
            command_set = drms.getQUAD_GY521_EEPROM();
        }
        else if (request == DRMS.LEV3_HEX){
            command_set = drms.getHEX_GY521_EEPROM();

        }
        Log.d(TAG,"Bluetooth Upload mode");
    }

    public STK500v1(Activity mActivity, UsbService usbservice, Handler mHandler, int request) {
//        this.outstream = outstream;
//        this.inputstream = inputstream;
        this.service = usbservice;
        this.mHandler = mHandler;
        this.mActivity = mActivity;
        this.request = request;
        mode = USB;
        conststk500 = new ConstantsStk500v1();
        Log.d(TAG,"USB Upload mode");
    }
        ...
        if (programMode_entered) {
            ...
            if(initializeEEPROM()) {
                if (writeHexFile()) {
                    ...
                    if(request == DRMS.LEV3_DRONE || request == DRMS.LEV3_HEX)
                        writeDataOnEEPROM();
