...
public class Cont1_1_2_Activity extends AppCompatActivity {
    ...
    private static final int MOLE1  = 20;
    private static final int MOLE2 = 21;
    private static final int MOLE3 = 22;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MOLE :
                    switch(msg.arg1){
                        case MOLE1 :
                            mole[0].setRotation(msg.arg2);
                            break;

                        case MOLE2 :
                            mole[1].setRotation(msg.arg2);
                            break;

                        case MOLE3 :
                            mole[2].setRotation(msg.arg2);
                            break;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cont1_1_2);
        ...
        textview = (TextView) findViewById(R.id.value1);
            ...
            switch(v.getId()){
                ...
                case R.id.onoff:
                    if(power_state == false && bluetoothService_obj.getState() == BluetoothService.STATE_CONNECTED && event.getAction() == MotionEvent.ACTION_DOWN){
                        ...
                        power_state = true;
                        sendMessage("#DRS^8^LEV1_1_R2^POWER_ON$",MODE_REQUEST);
                        ...
                        power.setImageDrawable(getResources().getDrawable(R.drawable.power_off));
                        power_state = false;
                        sendMessage("#DRS^9^LEV1_1_R2^POWER_OFF$",MODE_REQUEST);
                ...
                case R.id.upload:
                    if(event.getAction() == MotionEvent.ACTION_DOWN) {
                        ...
                        intent.putExtra("Firmware", "L1_1_R2");
                ...
                case R.id.start :
                    if(event.getAction() == MotionEvent.ACTION_DOWN
                            && bluetoothService_obj.getState() == BluetoothService.STATE_CONNECTED
                            && power_state == true) {
                        Toast.makeText(Cont1_1_2_Activity.this, "OK", Toast.LENGTH_SHORT).show();
                ...
                case R.id.end :
                    if(event.getAction() == MotionEvent.ACTION_DOWN
                        ...
                        sendMessage("#DRS^9^LEV1_1_R2^MOLE_STOP$", MODE_REQUEST);
                    }else if(event.getAction() == MotionEvent.ACTION_DOWN && bluetoothService_obj.getState() != BluetoothService.STATE_CONNECTED){
                        Toast.makeText(getApplicationContext(),"블루투스를 연결해주세요",Toast.LENGTH_SHORT).show();
                    }
                    break;
            ...
            while(running && step < 3 && bluetoothService_obj.getState() == BluetoothService.STATE_CONNECTED && power_state == true){
                    ...
                    switch (step) {
                        case 1:
                            ...
                            sendMessage("#DRS^14^LEV1_1_R2^M1" + mole_data[0] + "M2" + mole_data[1] + "M3" + mole_data[2] + "$", MODE_REQUEST);

                            int degree = 0;
                            while(degree <= 360){
                                if(mole_state[0] == true){
                                    mHandler.obtainMessage(MOLE,MOLE1,degree).sendToTarget();
                                }
                                if(mole_state[1] == true){
                                    mHandler.obtainMessage(MOLE,MOLE2,degree).sendToTarget();
                                }
                                if(mole_state[2] == true){
                                    mHandler.obtainMessage(MOLE,MOLE3,degree).sendToTarget();
                                }
                        ...
                        case 2:
                            ...
                            sendMessage("#DRS^13^LEV1_1_R2^M1" + mole_data[0] + "M2" + mole_data[1] + "M3" + mole_data[2] + "$", MODE_REQUEST);

                            int degree2 = 0;
                            while(degree2 <= 360){
                                if(mole_state[0] == true){
                                    mHandler.obtainMessage(MOLE,MOLE1,degree2).sendToTarget();
                                }
                                if(mole_state[1] == true){
                                    mHandler.obtainMessage(MOLE,MOLE2,degree2).sendToTarget();
                                }
                                if(mole_state[2] == true){
                                    mHandler.obtainMessage(MOLE,MOLE3,degree2).sendToTarget();
                                }
