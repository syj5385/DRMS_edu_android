...
public class DrsControllerManager {
    ...
    public static final int ANALOG = 7;

    public static final int R_T = 8;
    public static final int P_T = 9;
    public static final int Y_T = 10;
...
//                Log.d(TAG,"Controller data");
                ...
                if(checksum == data[data.length-1]) {
                    ...
                    drms.setRawRCDataRollPitch(roll,pitch);
                    drms.setRawRCDataYawThrottle(yaw,throttle);
                    ...
                    if(left_resistor >= 2000)
                        left_resistor = 2000;
                    if(left_resistor < 1000)
                        left_resistor = 1000;

                    right_resistor = read16(data[index++], data[index++]);
                    right_resistor = (3000-right_resistor);
                    if(right_resistor >= 2000)
                        right_resistor = 2000;
                    if(right_resistor < 1000)
                        right_resistor = 1000;
                ...
                //Log.d(TAG,"roll_t : " + roll_tream + "\tpitch_t : " + pitch_tream + "\tYaw_t : " + yaw_tream);
                    ...
                    setResistor(new int[]{left_resistor, right_resistor});

                    if(power == 200) {
                        mHandler.obtainMessage(CONT_DIGIAL, POWER, 0).sendToTarget();
                        if(drms.getRcdata()[7] == 1000){
                            drms.setRawRCDataAux(4,2000);
                        }
                        else if(drms.getRcdata()[7] == 2000){
    ...
    private void setRPYT(int roll, int pitch, int yaw, int throttle){
        this.roll = ((roll - 1500) * drms.getDRONE_SPEED() / 500) + 1500 ;
        this.pitch = ((pitch - 1500) * drms.getDRONE_SPEED() / 500) + 1500 ;
        this.yaw = ((yaw - 1500) * drms.getDRONE_SPEED() / 500) + 1500 ;
    ...
    private void setResistor(int[] resistor){
        drms.setResistor(resistor);
    }
...
//            service.sendBroadcast(new Intent(BTService.NEXT_DISPLAY));
    ...
    private void setDroneSpeed(int d1){
        if(d1 == 200){
            ...
            else if(drms.getDRONE_SPEED() == DRMS.slow){
                drms.setDRONE_SPEED(DRMS.middle);

            }
            else if(drms.getDRONE_SPEED() == DRMS.middle){
                drms.setDRONE_SPEED(DRMS.fast);

            }
            else if(drms.getDRONE_SPEED() == DRMS.fast){
                drms.setDRONE_SPEED(DRMS.veryfast);

            }
            else if(drms.getDRONE_SPEED() == DRMS.veryfast){
                drms.setDRONE_SPEED(DRMS.veryslow);

            }
            Log.d(TAG,"Throttle sensivive changed " + drms.getDRONE_SPEED());
            mHandler.obtainMessage(CONT_DIGIAL,D1,0).sendToTarget();
        }
    }
